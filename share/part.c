...
void part_burst(const float *p, const uint8_t *c)
{
    int i, n = 0;
...
static void part_draw(const float p[3], const uint8_t c[3],
                      float a, float r, float rx, float ry, float rz)
{
    glPushMatrix();
    {
        glTranslatef(r * p[0] / SCALE_VERTICES, p[1] / SCALE_VERTICES, r * p[2] / SCALE_VERTICES);
...
{
    float r = (float) timer_get() / 1000.f;
    int i;
