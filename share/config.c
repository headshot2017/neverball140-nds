...
void config_init(void)
{
    ...
    config_set_d(CONFIG_SOUNDTRACK,           DEFAULT_SOUNDTRACK);
        ...
        while (fgets(buf, MAXSTR, fp))
            if (sscanf(buf, "%s %s", key, val) == 2)
            {
                if      (strcmp(key, "fullscreen")            == 0)
                    ...
                    config_set_d(CONFIG_SOUNDTRACK,           atoi(val));
...
void config_save(void)
{
    ...
    if (dirty && (fp = fopen(config_user(USER_CONFIG_FILE), "w")))
    {
        ...
        fprintf(fp, "soundtrack           %d\n",
                option_d[CONFIG_SOUNDTRACK]);
...
int config_mode(int f, int w, int h)
{
	...
	glLoadIdentity();
	glScalef32(floattof32(SCALE_VERTICES), floattof32(SCALE_VERTICES), floattof32(SCALE_VERTICES));
...
int config_data_path(const char *path, const char *file)
{
    ...
    if (CONFIG_DATA && config_test(CONFIG_DATA, file))
    {
        ...
        printf("data path 3: '%s'\n", data_path);
        return 1;
    }

    printf("data path failed\n");
    return 0;
}
...
{
	int w = option_d[CONFIG_WIDTH];
    int h = option_d[CONFIG_HEIGHT];

	glMatrixMode(GL_PROJECTION);
	{
		glPushMatrix();
		glLoadIdentity();
		glOrthof32(0, inttof32(w), 0, inttof32(h), inttof32(-10), inttof32(10));
	}
	glMatrixMode(GL_MODELVIEW);
}
