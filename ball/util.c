    ...
    if ((jd = gui_hstack(id)))
    {
        ...
        if ((kd = gui_vstack(jd)))
        {
            ...
            if ((ld = gui_hstack(kd)))
            {
                ...
                {
                    for (j = 0; j < n - 2; j++)
                        time__t[j] = gui_clock(md, 359999, GUI_SML, 0);

                    time__t[j++] = gui_clock(md, 359999, GUI_SML, GUI_SE);
                    gui_space(md);
                    time__t[j++] = gui_clock(md, 359999, GUI_SML, GUI_RGT);
                }
...
void set_best_times(int level, int n)
{
    ...
    for (j = 0; j < n; j++)
    {
        gui_set_clock(time__t[j], level_time_t(level, j));
